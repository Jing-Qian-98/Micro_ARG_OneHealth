# PCA 分析将原始的高维数据通过线性变换组合，投影到维度较低的空间坐标系 (即主成分) 中，从而达到降维、简化数据结构的目的，展现样本的自然分布;对 PCA 分析得到的二维或三维数据作图，可以得知样本物种的主要分布特征，从而量化样本间的物种差异和相似度.
...
if(length(argv) > 0){
    if (length(argv) != 3) stop('The script needs three files')
    taxaAbdDir <- argv[1]
    map_file <- argv[2]
    level <- argv[3]
}
...
# 读取物种丰度表及预处理
taxaAbd <- read.table(taxaAbdDir, sep = "\t", header = T, row.names = 1, check.names = F, quote = "")
taxaAbd <- taxaAbd[ , -ncol(taxaAbd)]

# 转换相对丰度
taxaAbd_rel <- apply(taxaAbd,2,function(x){x / sum(x)})
taxaAbd_rel <- as.data.frame(taxaAbd_rel)

#重命名并行列转置
rownames(taxaAbd_rel) <- paste("X", as.character(1:nrow(taxaAbd_rel)), sep = "")
df <- t(taxaAbd_rel)
...
for (i in 1:ncol(map)){
    ...
    ## 输出PDF
    pdf(paste0("PCA_", level, "_", colnames(map)[i], ".pdf"),
        width =9,
        height =8,
        bg="white", onefile=F)
        grid.draw(q2) 
    ...
    ## 输出svg
    svg(paste0("PCA_", level, "_", colnames(map)[i], ".svg"),
        width =9,
        height =8,
        bg="white", onefile=F)
        grid.draw(q2) 
    ...
    ## 输出PNG
    png(paste0("PCA_", level, "_", colnames(map)[i], ".png"),
        width =9, 
        height =8,
        units="in",
        res=100,
    ...
    pca$Group <- NULL
    write.table(pca, paste("PCA_", level, "_", colnames(map)[i], ".txt", sep = ""), 
