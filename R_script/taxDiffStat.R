...
{
    meta <- fread(map, sep = "\t", header = T, stringsAsFactors = F, 
...
{
    df <- fread(data, sep = "\t", stringsAsFactors = F,
			header = TRUE, check.names = F, quote = "", data.table = F)
	df <- as.data.frame(df) 
	rownames(df) <- df[,1]
	df <- df[,-1]
	...
	# 根据样本频率过滤后可能为空，需要处理提示适当修改筛选条件
	if(length(rownames(df)) > 0){
		# 如果最后一列是字符串，则需要去除
		if(!is.numeric(df[, ncol(df)])){
			df <- df[-ncol(df)]
		}
		# 如果存在map表，则与丰度数据匹配样品名
		if(!is.null(map)){
...
add_errorbar=function(data,map,ngroup,index_order,x_theme, col, add_background)
{
	...
	p <- ggplot(diff_result, aes(varY, estimate, fill=Group))+
		...
		# scale_x_discrete(
		# 	limits = paste0(rep(index_order,each=2+length(levels(data$Group))),rep(0:(length(unique(data$Group))+1),t=length(index_order))),
		# 	breaks= paste0(index_order,rep(c(1:length(unique(data$Group)))[round((ngroup+1)/2)],t=length(index_order))),
		# 	labels= index_order,
		# 	expand = expansion(mult =0,add = 4)
		# )+ 
...
main <- function(conf){
    ...
    if (as.numeric(min(summary(map[,3])))<3 | as.numeric(length(unique(map[,3])))<2){  ###有分组小于3个样本	
		...
		if(length(gg) == 4){
			...
			p=ggplot(data=data.frame(x=1,y=1))+geom_text(x=0.5,y=0.75,size=12,label=c("显著性水平筛选后topN中没有差异显著的物种"))+
